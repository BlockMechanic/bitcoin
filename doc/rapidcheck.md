# RapidCheck property-based testing for Cypher Core

## Concept

Property-based testing is experimentally being added to Cypher Core with
...
If RapidCheck is installed, Cypher Core will automatically run the
...
3. Configure Cypher Core with RapidCheck.

    `cd` to the directory of your local cypher repository and run
...
4. Build Cypher Core with RapidCheck.
    ...
    Making all in src
    [...]
    CXX      test/gen/test_cypher-crypto_gen.o
    CXX      test/test_cypher-key_properties.o
    ```
...
That's it! You are now running property-based tests in Cypher Core.
