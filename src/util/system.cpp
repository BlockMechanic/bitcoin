...
const char * const CYPHER_CONF_FILENAME = "cypher.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "cypherd -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //cypher-tx using stdin
        std::string val;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Cypher";
#else
    // Unix
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", CYPHER_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Cypher Core copyright is not removed by accident
    if (copyright_devs.find("Cypher Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Cypher Core developers";
    }
