This directory contains the CypherQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Cypher UI.
...
ClientModel is used by the main application `cyphergui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a cypher:... link from a browser).
...
* UI elements like CypherAmountField, which inherit from QWidget.
* `cypherstrings.cpp`: automatically generated
* `cypherunits.(h/cpp)`: BTC / mBTC / etc handling
* `callback.h`
...
* don't change `local/cypher_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
4. Enter "cypher-qt" as project name, enter src/qt as location
5. Leave the file selection as it is
6. Confirm the "summary page"
7. In the "Projects" tab select "Manage Kits..."
8. Select the default "Desktop" kit and select "Clang (x86 64bit in /usr/bin)" as compiler
9. Select LLDB as debugger (you might need to set the path to your installation)
10. Start debugging with Qt Creator (you might need to the executable to "cypher-qt" under "Run", which is where you can also add command line arguments)
