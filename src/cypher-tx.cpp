...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " cypher-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  cypher-tx [options] <hex-tx> [commands]  Update hex-encoded cypher transaction\n" +
            "or:     cypher-tx [options] -create [commands]   Create hex-encoded cypher transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded cypher transaction
